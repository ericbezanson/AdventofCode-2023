...
// Main function to traverse the schematic
function traverseSchematic(inputGrid) {
    for (let y = 0; y < inputGrid.length; y++) {
        for (let x = 0; x < inputGrid[y].length; x++) {
            if (symbolFound(inputGrid[y][x]) && inputGrid[y][x] === "*") {
                gearIndex += 1;
                const gearKey = `gear${gearIndex}`;
                gears[gearKey] = [];
                for (let d = 0; d < directions.length; d++) {
                    lookAround(y, x, directions[d], gearKey);
                }
            }
        }
    }
    ...
    // Clean up gears with less than two arrays
...
function lookAround(yPosition, xPosition, direction, gearKey) {
    ...
    if (partNumberFound(inputGrid[newCoordinates[0]][newCoordinates[1]])) {
        completePartNumber(newCoordinates[0], newCoordinates[1], gearKey);
    }
}

// Function to find all numbers connected to symbol to create complete a part number
function completePartNumber(y, x, gearKey) {
    let partNumber = [];
    partNumber.push(inputGrid[y][x]);
    ...
    gears[gearKey].push(partNumber);
    sumPartNumber(partNumber);
}

// Function to check if a symbol is found
function symbolFound(inputString) {
    return /[^0-9.]/.test(inputString);
}

// Function to check if value is a number and not a symbol
function partNumberFound(inputString) {
    return /\d/.test(inputString);
}

// Function to sum the part number
function sumPartNumber(partNumber) {
    let total = '';
    for (let p = 0; p < partNumber.length; p++) {
        total += partNumber[p];
    }
    partNumberSum += parseInt(total);
}

// Function to process gears and calculate the total product
function processGears(gears) {
    let totalProduct = 0;

    for (const key in gears) {
        const gearArrays = gears[key];
        const fullNumbers = gearArrays.map(array => parseInt(array.join('')));
        
        // Multiply full numbers in each gear
        const gearProduct = fullNumbers.reduce((product, num) => product * num, 1);

        totalProduct += gearProduct;
    }

    console.log("ANSWER: total sum:", totalProduct);
}
