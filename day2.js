...
function part1(input) {
    ...
    // Create an object to store the games
    const gamesArray = {};
    ...
    // Split the game string based on semicolons
    const roundStrings = game.split(';');

    // Process each round
    roundStrings.forEach((roundString) => {
        const colors = [];
        // Extract the colors and counts from each round string
        const colorMatches = roundString.match(/(\d+)\s*([a-zA-Z]+)/g);

        // Process each color and count pair
        colorMatches.forEach((match) => {
        const [count, color] = match.split(/\s+/);
        const colorObject = { [color]: parseInt(count, 10) };
        colors.push(colorObject);
        });

        // Add the colors to the rounds array
        rounds.push(colors);
    });

        // Store the rounds in the object with the corresponding key
        let roundPass = isPossible(rounds);
        gamesArray[key] = roundPass;
    });

    // Filter out false games
    const filteredGames = Object.entries(gamesArray)
    .filter(([_, value]) => value)
    .map(([key]) => key);

    // Extract and sum the numbers from each remaining key
    const sum = filteredGames
    .map((key) => parseInt(key.match(/\d+/)[0], 10))
    .reduce((acc, curr) => acc + curr, 0);

    console.log('ANSWER: Sum of Possible Games:', sum);

    // Loop through each handful for each round
    function isPossible(rounds) {
        for (let i = 0; i < rounds.length; i++) {
        for (let x = 0; x < rounds[i].length; x++) {
            const color = Object.keys(rounds[i][x])[0];
            // check if number is possible based on color max rules
            if (rounds[i][x][color] > rules[color]) {
            return false;  // Return false immediately if any round fails
            }
        }
        }
        // If all rounds pass, return true
        return true;
    }
}
...
function part2(input) {
    ...
    roundStrings.forEach((roundString) => {
        ...
        // Store the rounds in the object with the corresponding key
        let power = findPower(rounds);
        gamesArray[key] = power;
    });


    function findPower(rounds) {
        const colorMax = {
            red: 0,
            green: 0,
            blue: 0
        ...
        for (let x = 0; x < rounds[i].length; x++) {
            ...
            switch(color) {
                case "red":
                   if (rounds[i][x][color] > colorMax[color]) {
                     colorMax[color] = rounds[i][x][color]
                   }
                  break;
                case "green":
                    if (rounds[i][x][color] > colorMax[color]) {
    ...
    const totalPower = findTotalPower(gamesArray) 

    function findTotalPower(games) {
        let sum = 0
    ...
    console.log('ANSWER: Total Power is :', totalPower);
}
