// Part 1 Function
const part1 = (input) => {
    // Split the input into an array of lines and filter out any empty lines
    ...
    for (const inputLine of inputArray) {
        // Initialize an array to store numbers for each line
        let numbers = [];
        ...
        for (let i = 0; i < inputLine.length; i++) {
            ...
            // If the character is a digit, add it to the numbers array
            if (isDigit(char)) {
                numbers.push(char);
            }
        }

        // Calculate the two-digit number from the first and last digits in the array
        const last = numbers.length - 1;
        const twoNum = `${numbers[0]}${numbers[last]}`;

        // Add the two-digit number to the total sum
        totalSum += Number(twoNum);
    }

    // Log the final total sum for Part 1
...
const part2 = (input) => {
    ...
    for (const inputLine of inputArray) {
        ...
        for (let i = 0; i < inputLine.length; i++) {
            ...
            for (let x = 0; x < numberWords.length; x++) {
                const numberWord = numberWords[x];
                const inputSubstring = inputLine.substring(i);

                // Check if the current substring starts with a number word
                if (inputSubstring.startsWith(numberWord)) {
                    // Add the corresponding number to the numbers array
                    numbers.push(x + 1);
                }
            }
        }
    ...
    // Log the final total sum for Part 2
